...
export default function Properties() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.minPrice === "" && filters.maxPrice === "" && 
        ...
        !selectedCampus /* 🆕 make 'no campus' also return all */) {
      return allListings;
    }
    ...
    if (selectedCampus && selectedCampus !== "all") {
      ...
      if (campus) {
        result = result.filter((l: any) => {
          ...
          const d = distanceBetween(l.lat, l.lng, campus.lat, campus.lng);
          return d <= campus.radius;
        });
      }
    }
    ...
    return result;
  // 🆕 include selectedCampus so the memo recomputes when campus changes
  }, [allListings, filters, bounds, selectedProperty, sortBy, selectedCampus]); // 🆕
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters, selectedCampus]); // 🆕 also reset on campus change
  ...
  const clearFilters = () => {
    ...
    setSelectedCampus(""); // 🆕 also clear campus
  };
...
<div className="space-y-3 relative z-[9999]">
  ...
  >
    ...
    {/* 🧠 Fix: Give SelectContent a very high z-index */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[800px]">
                <div className="lg:col-span-2">
                  <PropertyMap
                    ...
                    // 🆕 center the map on the selected campus, if any
                    center={selectedCampus && selectedCampus !== "all"
                      ? [
                          campuses.find((c) => c.id === selectedCampus)?.lat ?? 38.89,
                          campuses.find((c) => c.id === selectedCampus)?.lng ?? -77.1,
                        ]
                      : undefined}
