...
export function LeafletMap({
  ...
  const updateMarkers = useCallback(() => {
    ...
    // Only clear and update if properties actually changed
    const currentMarkerIds = new Set(markersRef.current.map(marker => (marker as any).propertyId));
    ...
    if (!needsUpdate) {
      // Only update selected marker styling if needed
      return;
    }
    ...
    propertiesToShow.forEach((property, index) => {
      if (property.latitude && property.longitude) {
        ...
        const marker = L.marker([property.latitude, property.longitude], { icon: propertyIcon }) as L.Marker & { propertyId: string };
    ...
    // Fit map logic - prioritize selected property over fitting all markers
  ...
  }, [propertiesToShow, selectedProperty, isLoaded, loading, onPropertySelect]);

  // Use the optimized updateMarkers function
  useEffect(() => {
    updateMarkers();
  }, [updateMarkers]);
